...
app.get("/", (req, res) => {
  res.send(`
    <html>
        ...
        <style>
          * {
          ...
          body {
            ...
            background: radial-gradient(circle at center, #1a1f3c, #0a0d1a);
            color: white;
            overflow: hidden;
          }
          ...
          .modal-btn {
            ...
            border-radius: 999px;
            border: none;
            background: linear-gradient(90deg, #ce5cf0, #9725a3);
            color: white;
            font-weight: bold;
            cursor: pointer;
            font-size: 14px;
      ...
      <body>
        <div class="container">
          <div class="title">STACKZ Player</div>
        ...
        <div class="modal" id="noticeModal">
          <div class="modal-box">
            ...
            <div class="modal-text">
              Stackz Player is a standalone media player.<br><br>
              It does not provide, host, or promote any content.<br><br>
              Users must supply their own legally obtained media.
            </div>
...
app.get("/get-movie-info", (req, res) => {
  ...
  safeFetch(
    `${dns}/player_api.php?username=${username}&password=${password}&action=get_vod_info&vod_id=${id}`,
    res
  );
});
...
app.get("/get-series-info", (req, res) => {
  const { dns, username, password, id } = req.query;
  safeFetch(
    `${dns}/player_api.php?username=${username}&password=${password}&action=get_series_info&series_id=${id}`,
    res
  );
});
