...
app.get("/subtitles", async (req, res) => {
  ...
  headers: {
    ...
    const wantedLangs = ["English", "Spanish", "French", "German", "Italian", "Portuguese", "Arabic"];
const subtitleResults = [];
const usedLangs = new Set();

const candidateResults = searchData.data.filter(item => {
  const attrs = item?.attributes || {};
  const lang = String(attrs.iso639 || "").toLowerCase();
  return wantedLangs.includes(lang);
});
...
for (const item of candidateResults) {
  ...
  if (!lang || usedLangs.has(lang)) continue;

  const feature = attrs.feature_details || {};
  if (normalizedSeason || normalizedEpisode) {
    const seasonOk = String(feature.season_number || "") === normalizedSeason;
    const episodeOk = String(feature.episode_number || "") === normalizedEpisode;
  ...
  if (!downloadResponse.ok || !downloadData.link) continue;

  subtitleResults.push({
    label: attrs.language || lang.toUpperCase(),
    lang: lang,
  ...
  if (subtitleResults.length >= 6) break;
}

if (!subtitleResults.length) {
  console.log("No subtitle download results for:", rawTitle);
  return res.json([]);
}

return res.json(subtitleResults);
  } catch (err) {
