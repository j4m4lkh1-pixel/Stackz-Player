...
function buildLiveStreamUrl(streamId) {
  ...
  if (avatar) {
    avatar.src = (activeProfile && activeProfile.avatar)
  ? (activeProfile.avatar.startsWith("images/") ? activeProfile.avatar : "images/" + activeProfile.avatar)
  ...
  (profile && profile.avatar)
    ? (profile.avatar.startsWith("images/") ? profile.avatar : "images/" + profile.avatar)
    : "images/Profile 1.jpg";
      } else {
        document.getElementById("profileIcon").src = "images/Profile 1.jpg";
      }
    }
...
function openProfileCard() {
  ...
  document.getElementById("profileCardImage").src = (profile && profile.avatar) ? profile.avatar : "images/Profile 1.jpg";
...
function applySavedTheme() {
  ...
  const finalTheme = savedTheme.startsWith("images/") ? savedTheme : "images/" + savedTheme;

  document.body.classList.add("themed-bg");
  document.body.style.backgroundImage = `url('${finalTheme}')`;
}
...
function loadThemeOptions() {
  ...
  this.src = theme.replace(".jpg", ".png");
    ...
    card.onclick = () => {
  const finalPath = img.getAttribute("src");
